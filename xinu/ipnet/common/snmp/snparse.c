...
int				len;
{
	 ...
	 */
	if (*packp++ != ASN1_SEQ || (totpacklen = a1readlen(packp, &lenlen)) < 0)
		return SYSERR;
	 ...
	 */
	if (packendp != packp + totpacklen || *packp++ != ASN1_INT ||
			*packp++ != SVERS_LEN || *packp++ != SVERSION ||
   			*packp++ != ASN1_OCTSTR || (commlen=a1readlen(packp, &lenlen)) < 0)
		return SYSERR;
...
/*		xp_tracev(80,"snparse: getmem  %d\n",sizeof(struct snbentry)); */
		...
		if (*packp++ != ASN1_SEQ || (varbindlen=a1readlen(packp, &lenlen)) < 0) 
			return SYSERR;

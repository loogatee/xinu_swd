...
/*
 *	check if the lengths of the oid's are the same, then check
...
struct snval
{
	...
	union
	{
		int 		sv_int;		/* one of: integer, counter, gauge, timeticks */
 ...
 *
 *	sizeof(struct snbentry) = 154
 */
struct snbentry
{
	struct oid		sb_oid;		/* object id in internal form */
	struct snval	sb_val;		/* value of the object */
	u_char			*sb_a1str;	/* ASN.1 string, contains objid and its value */
...
struct req_desc
{
	...
	int				err_stat_pos;	/* position err-status in ASN.1 encoding */
...
/*
 *	Codes for special cases regarding Address Table Entries
 */
#define SPECIAL_CASE_AT_1	2500	/* flag for special case #1 (see code) */
#define SPECIAL_CASE_AT_2	2501	/* flag for special case #2 (see code) */

/*
 *	Codes for special cases regarding Routing Table Entries
 */
#define SPECIAL_CASE_RT_1	2502	/* flag for special case #1 (see code) */

/*
 *	Codes for special cases regarding Address Entry Table
 */
#define SPECIAL_CASE_AE_1	2503	/* flag for special case #1 (see code) */
